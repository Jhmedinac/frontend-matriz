...
function App() {
  ...
  const cargarDatos = () => {
    fetch('https://eisenhower-backend.soltechn.cloud/api/tareas').then(res => res.json()).then(setTareas)
    fetch('https://eisenhower-backend.soltechn.cloud/api/categorias').then(res => res.json()).then(data => {
      setCategorias(data);
  ...
  const handleGuardarTarea = async (e) => {
    e.preventDefault()
    await fetch('https://eisenhower-backend.soltechn.cloud/api/tareas', {
  ...
  const completarTarea = async (id) => {
    await fetch(`https://eisenhower-backend.soltechn.cloud/api/tareas/${id}/completar`, { method: 'PUT' })
    cargarDatos()
  }

  const eliminarTarea = async (id) => {
    if (window.confirm('¿Eliminar esta tarea permanentemente?')) {
      await fetch(`https://eisenhower-backend.soltechn.cloud/api/tareas/${id}`, { method: 'DELETE' })
      cargarDatos()
    }
  }
  ...
  const handleGuardarCategoria = async (e) => {
    e.preventDefault()
    await fetch('https://eisenhower-backend.soltechn.cloud/api/categorias', {
  ...
  const eliminarCategoria = async (nombre) => {
    if (window.confirm(`¿Eliminar la categoría "${nombre}"? Las tareas existentes la conservarán en texto.`)) {
      await fetch(`https://eisenhower-backend.soltechn.cloud/api/categorias/${nombre}`, { method: 'DELETE' })
      cargarDatos()
    }
  }
