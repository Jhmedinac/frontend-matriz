...
function App() {
  ...
  const cargarDatos = () => {
    fetch('http://localhost:3000/api/tareas').then(res => res.json()).then(setTareas)
    fetch('http://localhost:3000/api/categorias').then(res => res.json()).then(data => {
      setCategorias(data);
  ...
  const completarTarea = async (id) => {
    await fetch(`http://localhost:3000/api/tareas/${id}/completar`, { method: 'PUT' })
    cargarDatos()
  }
  ...
  const eliminarCategoria = async (nombre) => {
    if (window.confirm(`¿Eliminar la categoría "${nombre}"? Las tareas existentes la conservarán en texto.`)) {
      await fetch(`http://localhost:3000/api/categorias/${nombre}`, { method: 'DELETE' })
      cargarDatos()
    }
  }
